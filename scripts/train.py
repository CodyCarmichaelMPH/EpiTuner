...
class SyndromicSurveillanceDataProcessor:
    ...
    def process_record(self, row: pd.Series) -> Dict[str, Any]:
        ...
        # Create a narrative syndromic surveillance record with clear section headers
...
CCDD Category: {row.get('CCDD Category', 'Not documented')}"""
        ...
        # Prepare the syndromic surveillance record content
        content = {
...
# Syndromic surveillance classification LoRA adapter
ADAPTER {lora_adapter_dir}

# System prompt for medical classification
SYSTEM "You are an expert syndromic surveillance analyst trained to classify syndromic surveillance records. You analyze patient data and provide accurate classifications based on the specified criteria."
"""
...
1. Review the syndromic surveillance record carefully
...
3. **If REVISED/REDO, provide improved classification:**
   ...
   - **Rationale:** [Provide a detailed, evidence-based explanation that includes:
     ...
     - Reference to specific sections of the syndromic surveillance record]"""
    ...
    def _simulate_validation(self, record: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if len(rationale) < 50 or 'not documented' in rationale.lower():
            return {
                ...
                'rationale': f"Based on the syndromic surveillance record: {narrative[:200]}... [Enhanced rationale would be generated by the model]"
            }
...
def main():
    ...
    processor = SyndromicSurveillanceDataProcessor(args.template, args.topic)
